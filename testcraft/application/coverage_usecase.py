...
from ..adapters.io.file_discovery import FileDiscoveryService, FileDiscoveryError
    ...
    async def measure_and_report(
        ...
        **kwargs: Any
        ...
        with self._telemetry.create_span(
            ...
            try:
                logger.info("Starting coverage measurement and reporting for project: %s", project_path)
                ...
                span.set_attribute("files_discovered", len(discovery_result["source_files"]))
                ...
                span.set_attribute("overall_coverage", coverage_summary.get("overall_line_coverage", 0))
                ...
                await self._record_telemetry_and_metrics(span, coverage_data, coverage_summary)
                ...
                results = {
                    ...
                    "overall_line_coverage": coverage_summary.get("overall_line_coverage", 0),
                    "overall_branch_coverage": coverage_summary.get("overall_branch_coverage", 0),
                    "metadata": {
                        "project_path": str(project_path),
                        "config_used": self._config,
                        "timestamp": discovery_result.get("timestamp"),
                        "coverage_method": getattr(self._coverage, 'get_last_method_used', lambda: 'unknown')()
                    }
                }
                ...
                raise CoverageUseCaseError(
                    f"Coverage measurement failed: {e}",
                    cause=e
                )
        ...
        with self._telemetry.create_child_span("sync_state_and_discover") as span:
            try:
                ...
                if source_files:
                    ...
                    discovered_source_files = self._file_discovery.filter_existing_files(source_files)
                    span.set_attribute("discovery_method", "provided_source_files")
                else:
                    # Discover source files using file discovery service
                    discovered_source_files = self._file_discovery.discover_source_files(
                        project_path,
                        include_test_files=self._config.get('include_test_files', False)
                    )
                ...
                if self._config.get('include_test_files', False):
                    if test_files:
                        discovered_test_files = self._file_discovery.filter_existing_files(test_files)
                    else:
                        discovered_test_files = self._file_discovery.discover_test_files(project_path)
    ...
    async def _measure_coverage(
        ...
        with self._telemetry.create_child_span("measure_coverage") as span:
            try:
                ...
                span.set_attribute("files_measured", len(coverage_data))
                span.set_attribute("measurement_method", 
        ...
        try:
            ...
            metrics = [
                MetricValue(
                    ...
                    unit="count",
                    labels={"method": getattr(self._coverage, 'get_last_method_used', lambda: 'unknown')()},
                ...
                MetricValue(
                    ...
                    value=sum(
                        1 for result in coverage_data.values() 
            ...
            span.set_attribute("overall_line_coverage", coverage_summary.get("overall_line_coverage", 0))
            span.set_attribute("overall_branch_coverage", coverage_summary.get("overall_branch_coverage", 0))
            span.set_attribute("coverage_method", 
                             getattr(self._coverage, 'get_last_method_used', lambda: 'unknown')())
    ...
    def calculate_coverage_delta(
        ...
        try:
            ...
            return {
                ...
                "trend": "improving" if line_delta > 0 else "declining" if line_delta < 0 else "stable"
            }

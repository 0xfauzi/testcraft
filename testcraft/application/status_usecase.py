...
class StatusUseCase:
        ...
        **kwargs: Any
        ...
        with self._telemetry.create_span(
            ...
            try:
                ...
                # Apply sorting
                sorted_history = self._sort_history(filtered_history, sort_by, sort_order)
                ...
                raise StatusUseCaseError(
                    f"Filtered history retrieval failed: {e}",
                    cause=e
                )
    ...
    async def _get_current_state(self) -> Dict[str, Any]:
        ...
        with self._telemetry.create_child_span("get_current_state") as span:
            try:
                ...
                current_state = {
                    "last_generation_timestamp": last_generation_run.get("last_run_timestamp"),
                    "last_coverage_timestamp": last_coverage_run.get("last_coverage_run_timestamp"),
                    "generation_summary": last_generation_run.get("generation_summary", {}),
                    "refinement_summary": last_generation_run.get("refinement_summary", {}),
                    "coverage_improvement": last_generation_run.get("coverage_improvement", {}),
                    ...
                    "state_health": {
                        ...
                        "data_freshness_hours": self._calculate_data_freshness(last_generation_run),
                    }
                }
                ...
                return {
                    "error": str(e),
                    "state_health": {"has_generation_data": False, "has_coverage_data": False}
                }
            ...
            try:
                ...
                if "last_generation_run" in generation_state:
                    ...
                    history_entry = {
                        ...
                        "status": "success" if last_run.get("generation_summary", {}).get("successful_generations", 0) > 0 else "partial",
                        "files_processed": last_run.get("generation_summary", {}).get("total_files_processed", 0),
                        "tests_generated": last_run.get("generation_summary", {}).get("successful_generations", 0),
                        "tests_refined": last_run.get("refinement_summary", {}).get("successful_refinements", 0),
                        "coverage_delta": last_run.get("coverage_improvement", {}).get("line_coverage_delta", 0),
                ...
                if "last_coverage_run" in coverage_state:
                    ...
                    history_entry = {
                        ...
                        "coverage_percentage": coverage_run.get("coverage_summary", {}).get("overall_line_coverage", 0),
        ...
        with self._telemetry.create_child_span("get_summary_statistics") as span:
            try:
                ...
                recent_entries = [
                    ...
                    datetime.fromtimestamp(entry["timestamp"]) > cutoff_time
                ]
                
                # Calculate statistics
                total_runs = len(recent_entries)
                successful_runs = len([e for e in recent_entries if e.get("status") == "success"])
                ...
                # Coverage statistics
                coverage_entries = [e for e in recent_entries if "coverage_percentage" in e]
                avg_coverage = (
                    sum(e["coverage_percentage"] for e in coverage_entries) / len(coverage_entries)
                    if coverage_entries else 0
                )
                ...
                statistics = {
                    ...
                    "recent_activity": {
                        "last_24h": len([e for e in recent_entries 
            ...
            try:
                ...
                for file_path in discovered_files:
                    ...
                    try:
                        ...
                        file_age_days = (datetime.now().timestamp() - mod_time) / (24 * 3600)
                    except Exception:
                        file_age_days = None
    ...
    def _calculate_data_freshness(self, last_run: Dict[str, Any]) -> Optional[float]:
    ...
    def _calculate_activity_trend(self, recent_entries: List[Dict[str, Any]]) -> str:
        ...
        recent_count = len([e for e in recent_entries[:len(recent_entries)//2]])
        older_count = len([e for e in recent_entries[len(recent_entries)//2:]])

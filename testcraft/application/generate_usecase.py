...
from ..ports.telemetry_port import TelemetryPort, SpanKind, MetricValue
...
class GenerateUseCase:
    ...
    async def generate_tests(
        ...
        target_files: Optional[List[Union[str, Path]]] = None,
        **kwargs: Any
    ) -> Dict[str, Any]:
        """
        ...
        with self._telemetry.create_span(
            ...
            try:
                ...
                span.set_attribute("initial_coverage", initial_coverage.get("overall_line_coverage", 0))
                ...
                context_data = await self._gather_project_context(
                    project_path, files_to_process
                ) if self._config['enable_context'] else {}
                ...
                span.set_attribute("final_coverage", final_coverage.get("overall_line_coverage", 0))
                span.set_attribute("coverage_delta", coverage_delta.get("line_coverage_delta", 0))
        ...
        with self._telemetry.create_child_span("sync_state_and_discover") as span:
            try:
                ...
                if target_files:
                    ...
                    files = [Path(f) for f in self._file_discovery.filter_existing_files(file_paths)]
                ...
                return {
                    ...
                    "timestamp": span.get_trace_context().trace_id if span.get_trace_context() else None,
    ...
    async def _measure_initial_coverage(
        ...
        with self._telemetry.create_child_span("measure_initial_coverage") as span:
            try:
                ...
                span.set_attribute("overall_coverage", summary.get("overall_line_coverage", 0))
    ...
    async def _decide_files_to_process(
        ...
        with self._telemetry.create_child_span("decide_files_to_process") as span:
            ...
            try:
                ...
                raise GenerateUseCaseError(
                    f"File processing decision failed: {e}",
                    cause=e
                )
    ...
    async def _file_needs_processing(
        ...
        try:
            ...
            has_existing_tests = any(test_file.exists() for test_file in potential_test_files)
            ...
            return not has_existing_tests or current_coverage < self._config['coverage_threshold']
            
        except Exception as e:
            logger.warning("Failed to check if file %s needs processing: %s", file_path, e)
    ...
    async def _build_generation_plans(
        ...
        with self._telemetry.create_child_span("build_generation_plans") as span:
            ...
            try:
                ...
                raise GenerateUseCaseError(
                    f"Generation plan creation failed: {e}",
                    cause=e
                )
    ...
    async def _execute_test_generation(
        self,
        generation_plans: List[TestGenerationPlan],
        ...
        with self._telemetry.create_child_span("execute_test_generation") as span:
            try:
                ...
                span.set_attribute("successful_generations", 
                ...
                raise GenerateUseCaseError(
                    f"Test generation execution failed: {e}",
                    cause=e
                )
    ...
    async def _get_relevant_context(
        ...
        try:
            ...
            query_parts = [element.name for element in plan.elements_to_test[:3]]  # Limit query size
    ...
    async def _write_test_files(
        ...
        with self._telemetry.create_child_span("write_test_files") as span:
            ...
            try:
                ...
                span.set_attribute("files_written", 
                                 sum(1 for r in write_results if r["success"]))
            ...
            try:
                ...
                for write_result in successful_writes:
                    ...
                    try:
                        ...
                        logger.warning("Refinement failed for %s: %s", test_file_path, e)
                ...
                span.set_attribute("files_refined", 
                ...
                raise GenerateUseCaseError(
                    f"Test execution and refinement failed: {e}",
                    cause=e
                )
        ...
        with self._telemetry.create_child_span("refine_test_file") as span:
            ...
            for iteration in range(max_iterations):
                ...
                try:
                    ...
                    span.set_attribute(f"iteration_{iteration}_pytest_returncode", pytest_result["returncode"])
                    ...
                    source_context = await self._build_source_context_for_test(test_file)
                    ...
                    logger.warning("Refinement iteration %d failed for %s: %s", 
                                 iteration + 1, test_file_path, e)
    ...
    async def _build_source_context_for_test(self, test_file: Path) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            try:
                ...
                if hasattr(self, '_context'):
                    try:
                        ...
                        index_result = self._context.index(test_file, content=context["test_content"])
                        ...
                        for related_file_path in related_context.get("related_files", []):
                            ...
                            if related_path.exists() and related_path.suffix == '.py':
                                try:
                                    ...
                                    logger.warning("Failed to read related file %s: %s", related_path, e)
                        
                        # Step 4: Build intelligent retrieval queries from test context
                        # Extract key terms from test file for context retrieval
                        retrieval_queries = self._extract_test_context_queries(test_file, context["test_content"])
                            ...
                            try:
                                ...
                                if retrieval_result.get('results'):
                                    context["retrieved_context"].append({
                                        "query": query,
                                        "results": retrieval_result['results'][:2]  # Limit results
                                    })
                                    
                            except Exception as e:
                                logger.warning("Context retrieval failed for query '%s': %s", query, e)
                
                        # Step 5: Add import-based source file discovery
                        # Look for source files corresponding to internal dependencies
                        for dep in internal_deps:
                            potential_source_paths = self._find_source_files_for_module(test_file, dep)
                        ...
                        logger.warning("Context port analysis failed for %s: %s", test_file, e)
            ...
            try:
                context["project_structure"] = self._build_directory_tree(
                    test_file.parent.parent if test_file.parent != test_file.parent.parent 
    ...
    def _extract_test_context_queries(self, test_file: Path, test_content: str) -> List[str]:
        """
        ...
        try:
                    ...
                    if isinstance(node, ast.FunctionDef) and node.name.startswith('test_'):
                        ...
                        for child in ast.walk(node):
                            if isinstance(child, ast.Constant) and isinstance(child.value, str):
                                ...
                                if 3 <= len(child.value) <= 30 and child.value.isalnum():
                                    queries.append(child.value)
                                    
            except SyntaxError as e:
                logger.warning("Could not parse test file %s for query extraction: %s", test_file, e)
            ...
            logger.warning("Failed to extract context queries from %s: %s", test_file, e)
            return [test_file.stem.replace('test_', '').replace('_', ' ')]

    def _find_source_files_for_module(self, test_file: Path, module_name: str) -> List[Path]:
        """
        ...
        try:
            ...
            logger.warning("Failed to find source files for module %s: %s", module_name, e)
            return []

    async def _measure_final_coverage(
        self,
        source_files: List[Path]
    ) -> Dict[str, Any]:
        """
        ...
        with self._telemetry.create_child_span("measure_final_coverage") as span:
            try:
                ...
                span.set_attribute("files_measured", len(coverage_data))
                span.set_attribute("final_coverage", summary.get("overall_line_coverage", 0))
        ...
        try:
            ...
            state_data = {
                ...
                "generation_summary": {
                    ...
                    "failed_generations": sum(1 for r in generation_results if not r.success)
                },
                "refinement_summary": {
                    "total_files_refined": len(refinement_results),
                    "successful_refinements": sum(1 for r in refinement_results if r.get("success", False))
                },
    ...
    async def _record_telemetry_and_costs(
        ...
        try:
            ...
            span.set_attribute("successful_refinements", 
                             sum(1 for r in refinement_results if r.get("success", False)))

...
class GenerateUseCase:
            ...
            try:
                ...
                if self._config["immediate_refinement"]:
                    ...
                    if self._config["enable_refinement"]:
                        refinement_results = await self._execute_and_refine_tests(write_results)
    ...
    async def _process_plans_immediately(
        ...
        for i in range(0, len(generation_plans), batch_size):
            ...
            batch_tasks = [
                self._process_plan_immediate(plan, context_data) 
                for plan in batch
            ]
            ...
            if not generation_result.success or not generation_result.content:
                ...
                if self._current_status_tracker:
                    self._current_status_tracker.update_file_status(
                        ...
                        step=generation_result.error_message or "Test generation failed",
                        progress=0.0
                    )
                return result
                    ...
                    if refinement_result.get("success", False):
                        ...
                        if self._current_status_tracker:
                            self._current_status_tracker.update_file_status(
                                ...
                                step=refinement_result.get('error', 'Maximum iterations reached'),
                                progress=0.0
                            )
    ...
    async def _record_per_file_telemetry(self, file_result: dict[str, Any], file_path: str) -> None:
        """
    ...
    def _record_module_path_telemetry(self, module_path_info: dict[str, Any], source_path: Path) -> None:
        """
    ...
    async def _refine_test_file_immediate(self, test_file_path: str) -> dict[str, Any]:
        ...
        try:
            ...
            async def build_context_for_refinement(test_file: Path, test_content: str) -> dict[str, Any] | None:
                return self._context_assembler.context_for_refinement(test_file, test_content)

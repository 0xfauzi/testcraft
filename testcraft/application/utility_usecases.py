...
class UtilityUseCase:
    ...
    def __init__(
        ...
        cost_port: Optional[CostPort] = None,
            ...
            try:
                ...
                if force_reload:
                    try:
                        ...
                        sync_results["state_changes"]["loaded_keys"] = load_result.get("loaded_keys", [])
                        span.set_attribute("keys_loaded", len(load_result.get("loaded_keys", [])))
                    except Exception as e:
                ...
                try:
                    ...
                    span.set_attribute("validation_issues", len(validation_result.get("issues", [])))
                except Exception as e:
                ...
                if persist_after_sync:
                    try:
                        ...
                        sync_results["operations_performed"].append("persist_to_storage")
                        sync_results["state_changes"]["persisted_keys"] = persist_result.get("persisted_keys", [])
                        span.set_attribute("keys_persisted", len(persist_result.get("persisted_keys", [])))
                    except Exception as e:
                ...
                raise UtilityUseCaseError(
                    f"State synchronization failed: {e}",
                    cause=e
                )

    async def reset_state(
        self,
        state_categories: Optional[List[str]] = None,
            ...
            try:
                ...
                if create_backup:
                    try:
                        ...
                        reset_results["backup_location"] = backup_data.get("backup_location")
                        span.set_attribute("backup_created", True)
                        logger.info("State backup created at: %s", backup_data.get("backup_location"))
                    except Exception as e:
                ...
                categories_to_reset = state_categories or ["generation", "coverage", "telemetry"]
                
                for category in categories_to_reset:
                    try:
                        clear_result = self._state.clear_state(category)
                ...
                span.set_attribute("categories_reset", len(reset_results["categories_reset"]))
        ...
        with self._telemetry.create_span(
            ...
            try:
                ...
                results = {
                    ...
                    "metadata": {
                        "detail_level": self._config.get('env_info_detail_level', 'standard'),
                ...
                raise UtilityUseCaseError(
                    f"Environment info collection failed: {e}",
                    cause=e
                )
        ...
        **kwargs: Any
        ...
        with self._telemetry.create_span(
            ...
            try:
                ...
                span.set_attribute("within_limits", limit_status.get("within_limits", True))
                ...
                raise UtilityUseCaseError(
                    f"Cost summary generation failed: {e}",
                    cause=e
                )
    ...
    async def _collect_debug_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            debug_state["metadata"] = {
                ...
                "total_state_keys": len(generation_state) + len(coverage_state) + len(telemetry_state),
                "state_health": {
    ...
    def _collect_config_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "sensitive_data_included": self._config.get('include_sensitive_data', False)
        }
    ...
    def _format_debug_output(self, debug_state: Dict[str, Any], output_format: str) -> str:
    ...
    async def _validate_state_consistency(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for key, value in all_state.items():
                if value is None:
                    validation_result["warnings"].append(f"Null value for state key: {key}")
                elif isinstance(value, dict) and not value:
                    validation_result["warnings"].append(f"Empty dictionary for state key: {key}")
    ...
    async def _calculate_cost_projections(
        ...
        try:
            ...
            projections = {
                ...
                "projected_monthly": current_cost * 30 if time_period == "daily" else current_cost,
                "projected_yearly": current_cost * 365 if time_period == "daily" else current_cost * 12,

...
class PackagingInfo:
    ...
    def get_canonical_import(self, file_path: Path) -> Optional[str]:
    ...
    def _detect_source_roots(
        ...
        for dir_name in common_source_dirs:
            ...
            if src_dir.exists() and src_dir.is_dir():
                ...
                if any(
                    (src_dir / item).is_dir() and (src_dir / item / "__init__.py").exists()
...
class EntityInterfaceDetector:
    ...
    def detect_entities(file_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "has_orm_models": any(
                    info.get("kind") == "sqlalchemy.Model" 
    ...
    @staticmethod
    def _analyze_class(node: ast.ClassDef, content: str) -> Optional[Dict[str, Any]]:
        ...
        for base in node.bases:
            ...
            if base_name:
                if "Model" in base_name or "Base" in base_name:
                    ...
                    info["instantiate_real"] = True  # Pydantic models are safe to instantiate
        ...
        for class_node in node.body:
            if isinstance(class_node, ast.FunctionDef) and class_node.name == "__init__":
                info["constructor_signature"] = EntityInterfaceDetector._get_function_signature(
                    class_node, content
                )
                break
    ...
    @staticmethod
    def _get_function_signature(node: ast.FunctionDef, content: str) -> Optional[str]:

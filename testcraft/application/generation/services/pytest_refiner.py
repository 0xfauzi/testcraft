        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                if "/" in match or "\\" in match:
                    ...
                    module_path = (
                        match.replace("\\", "/").replace(".py", "").replace("/", ".")
                    )
            ...
            if runner in failure_output.lower():
                ...
                for pattern in module_patterns:
                    ...
                    for match in matches:
                        if (
                            len(match.split(".")) >= 2
                            and not match.startswith("test")
                            and match not in ["sys.path", "os.path", "json.loads"]
                        ):
    ...
    async def _mark_test_with_bug_info(
        ...
        try:
            if not test_file.exists():
                logger.warning(
                    "Test file %s does not exist, cannot mark with bug info", test_file
                )
                return
            ...
            bug_description = bug_detection.get(
...
'''
    ...
    async def _annotate_failed_test(
        ...
        try:
            if not test_file.exists():
                logger.warning(
                    "Test file %s does not exist, cannot annotate", test_file
                )
                return
            ...
            if "TESTCRAFT_FAILED_REFINEMENT_GUIDE" in current_content:
                logger.debug(
                    "Test file %s already has failed refinement annotation", test_file
                )
                return

            # Extract enhanced failure analysis
            failing_tests = self._extract_failing_tests_from_output(
                failure_output or ""
            )
                ...
                if failure_context.get("key_messages"):
                    banner_lines.extend(
                        [
                            ...
                            "\n".join(
                                f"  • {msg}"
                                for msg in failure_context["key_messages"][:3]
                            ),
                            "",
                        ]
                    )
                    ...
                    trimmed_failure = failure_output[
                        : self._config.annotation_max_failure_chars
                    ]
            ...
            if self._config.include_llm_fix_instructions:
                ...
                banner_lines.extend(
                    [
                        ...
                        self._format_fix_instructions_as_todos(
                            extra["preflight_suggestions"]
                        ),
                        "",
                    ]
                )
            ...
            if self._telemetry:
                with self._telemetry.create_child_span(
                    "failed_annotation_added"
                ) as span:
                    span.set_attribute("failed_annotation_added", True)
                    span.set_attribute(
                        "annotation_style", self._config.annotation_style
                    )
                    span.set_attribute(
                        "annotation_placement", self._config.annotation_placement
                    )
                    span.set_attribute(
                        "included_llm_instructions", bool(fix_instructions)
                    )
    ...
    def _format_fix_instructions_as_todos(self, instructions: str) -> str:
        ...
        for line in lines:
            ...
            if stripped.startswith("- ") or stripped.startswith("* "):
                ...
                # Handle "1. " or "1) " style
                todo_lines.append(
                    f"- [ ] {stripped.split('. ', 1)[-1].split(') ', 1)[-1].strip()}"
                )
            else:
        ...
        for line in lines:
            for indicator in key_indicators:
                if indicator in line:
                    ...
                    if (
                        len(clean_line) > 15
                        and clean_line not in context["key_messages"]
                        and len(context["key_messages"]) < 5
    ...
    def _generate_enhanced_fix_instructions(
        self,
        ...
        if failing_tests:
            ...
            if len(failing_tests) > 5:
                instructions.append(
                    f"   ... and {len(failing_tests) - 5} more tests failing"
                )
            instructions.append("")
            ...
            for import_error in failure_context["import_errors"][:2]:
                if "No module named" in import_error:
                    ...
                    if module_match:
                        ...
                        instructions.append(
                            f"   • Missing module '{module}' - add proper mock:"
                        )
                        instructions.append(
                            f"     with patch('{module}') as mock_{module.replace('.', '_')}:"
                        )
                    else:
                        instructions.append(
                            f"   • Fix import issue: {import_error[:80]}"
                        )
                else:
        ...
        if failure_context.get("syntax_errors"):
            ...
            instructions.append(
                "   • Check for missing imports, incorrect indentation, or typos"
            )
            instructions.append("")
        ...
        if failure_context.get("assertion_details"):
            ...
            instructions.append(
                f"   • Failed: {failure_context['assertion_details'][:100]}"
            )
        ...
        if failing_tests:
            ...
            instructions.append(
                f"{step}. Focus on these specific tests: {', '.join(test_names)}"
            )
            step += 1

        if failure_context.get("import_errors"):
            instructions.append(
                f"{step}. Fix import issues FIRST (they often block other fixes)"
            )
            step += 1
        ...
        if failure_context.get("error_types"):
            ...
            if "Import" in primary_error or "Module" in primary_error:
                instructions.append(
                    f"{step}. Add proper mocks for missing modules using @patch decorator"
                )
            elif "Assertion" in primary_error:
                instructions.append(
                    f"{step}. Debug assertion: check expected values vs actual mock returns"
                )
            elif "AttributeError" in primary_error:
                instructions.append(
                    f"{step}. Fix attribute names and verify mock object configuration"
                )
            elif "SyntaxError" in primary_error:
                instructions.append(
                    f"{step}. Check Python syntax, imports, and indentation"
                )
            else:
                instructions.append(
                    f"{step}. Address {primary_error} by reviewing test setup"
                )
            step += 1

        instructions.append(
            f"{step}. Test fix with: pytest {failing_tests[0] if failing_tests else '[test_file]'} -v"
        )
        instructions.append(
            f"{step + 1}. Verify mocks are properly configured and don't interfere with each other"
        )

        # Add meaningful LLM instructions if available
        if (
            original_instructions
            and "No obvious canonicalization issues" not in original_instructions
            and len(original_instructions.strip()) > 20
        ):
            instructions.append("")
...
"""
    ...
    async def refine_until_pass(
        ...
        test_path: str,
        max_iterations: int,
        build_source_context_fn: Callable[
            [Path, str], Awaitable[dict[str, Any] | None]
        ],
    ) -> dict[str, Any]:
        """
        ...
        async with self._refine_semaphore:
            with self._telemetry.create_child_span("refine_test_file") as span:
                ...
                for iteration in range(max_iterations):
                    ...
                    if self._status_tracker:
                        if iteration == 0:
                            self._status_tracker.update_file_status(
                                ...
                                FileStatus.TESTING,
                                operation="Testing",
                                step="Running initial pytest check",
                    ...
                    try:
                        ...
                        if pytest_result["returncode"] == 0:
                            ...
                            combined_output = pytest_result.get(
                                "stdout", ""
                            ) + pytest_result.get("stderr", "")

                            if (
                                self._config.fail_on_xfail_markers
                                and self.detect_xfail_in_output(combined_output)
                            ):
                                logger.warning(
                                ...
                                await self._mark_test_with_bug_info(
                                    test_file, xfail_bug_detection, combined_output
                                )
                        ...
                        if pytest_result.get("unrefinable", False):
                            ...
                            if self._config.annotate_failed_tests:
                                await self._annotate_failed_test(
                                    test_file=Path(test_path),
                                    failure_output=self.format_pytest_failure_output(
                                        pytest_result
                                    ),
                            ...
                            return {
                                ...
                                "last_failure": self.format_pytest_failure_output(
                                    pytest_result
                                ),
                            }
                        ...
                        try:
                            ...
                            logger.warning(
                                "Failed to read test file %s: %s", test_file, e
                            )
                            test_content = ""

                        source_context = await build_source_context_fn(
                        ...
                        refine_result = self._refine.refine_from_failures(
                            ...
                            max_iterations=self._config.max_retries
                            + 1,  # Use config max_retries
                            timeout_seconds=int(
                        ...
                        last_active_import_path = refine_result.get(
                            "active_import_path", ""
                        )
                        ...
                        if final_status == "prod_bug_suspected":
                            suspected_bug_desc = refine_result.get(
                                "suspected_prod_bug", "Production bug detected by LLM"
                            )
                            logger.warning(
                            ...
                            await self._mark_test_with_bug_info(
                                test_file, bug_detection, failure_output
                            )

                            # Create a bug report file
                            await self._create_bug_report(
                                test_path, bug_detection, failure_output, iteration + 1
                            )
                        ...
                        if final_status == "llm_invalid_output":
                            ...
                            span.set_attribute(
                                "invalid_output_iteration", iteration + 1
                            )
                        ...
                        if final_status in no_change_statuses:
                            ...
                            stop_on_no_change = (
                                getattr(self._config, "stop_on_no_change", True)
                                if hasattr(self, "_config")
                                else True
                            )

                            # Also check cosmetic handling config
                            treat_cosmetic_as_no_change = (
                                getattr(
                                    self._config, "treat_cosmetic_as_no_change", True
                                )
                                if hasattr(self, "_config")
                                else True
                            )
                            ...
                            if (
                                final_status == "content_cosmetic_noop"
                                and not treat_cosmetic_as_no_change
                            ):
                                logger.info(
                            ...
                            if stop_on_no_change:
                                ...
                                reason = status_messages.get(
                                    final_status,
                                    "LLM refinement made no meaningful changes",
                                )
                                ...
                                return {
                                    ...
                                    "no_change_details": {
                                        ...
                                        "reason": reason,
                                        "diff_snippet": refine_result.get(
                                            "diff_snippet", ""
                                        ),
                                    },
                                }
                        ...
                        if final_status == "syntax_error":
                            ...
                            return {
                                ...
                                "error": refine_result.get(
                                    "error", "Syntax error in refined content"
                                ),
                                "last_failure": failure_output,
                            }
                ...
                try:
                    ...
                    final_status = (
                        "passed" if final_pytest["returncode"] == 0 else "failed"
                    )

...
class LLMCredentials(BaseModel):
    ...
    def has_azure_openai_credentials(self) -> bool:
        ...
        return (self.get_azure_openai_api_key() is not None and 
    ...
    def has_aws_bedrock_credentials(self) -> bool:
        ...
        return (self.get_aws_access_key_id() is not None and
                self.get_aws_secret_access_key() is not None and
                self.aws_region is not None)
...
class CredentialManager:
    ...
    def __init__(self, config_overrides: Optional[Dict[str, Any]] = None):
        ...
        self._credentials_cache: Optional[LLMCredentials] = None
    ...
    def load_credentials(self, reload: bool = False) -> LLMCredentials:
        ...
        try:
            ...
            for field_name in self.ENV_MAPPINGS.keys():
                ...
                if config_key in self.config_overrides and field_name not in credential_data:
            ...
            if available:
                logger.info(f"Loaded credentials for LLM providers: {', '.join(available)}")
            else:
                logger.warning("No LLM provider credentials found - adapters will fail without API keys")
    ...
    def _load_from_env(self, env_vars: list[str]) -> Optional[str]:
    ...
    def get_provider_credentials(self, provider: str) -> Dict[str, Any]:
        ...
        if provider == 'openai':
            if not credentials.has_openai_credentials():
                raise CredentialError("OpenAI credentials not found. Set OPENAI_API_KEY environment variable.")
            return {
            ...
            if not credentials.has_anthropic_credentials():
                raise CredentialError("Anthropic credentials not found. Set ANTHROPIC_API_KEY environment variable.")
            return {
    ...
    def create_env_template(self, filepath: Optional[Union[str, Path]] = None) -> Path:
...
def load_credentials(config_overrides: Optional[Dict[str, Any]] = None) -> LLMCredentials:

...
class TelemetryAdapterRegistry:
    ...
    def get_adapter_class(self, name: str) -> type[TelemetryPort]:
        ...
        if not adapter_class:
            ...
            raise ValueError(
                f"Unknown telemetry adapter '{name}'. Available: {available}"
            )
...
class TelemetryContextManager:
    ...
    def __init__(
        self, telemetry: TelemetryPort, operation_name: str, **span_attributes
    ):
        """
    ...
    def __enter__(self):
        ...
        self._span_context = self.telemetry.create_span(
            self.operation_name, attributes=self.span_attributes
        ).__enter__()
...
def create_telemetry_context(
    telemetry: TelemetryPort, operation_name: str, **span_attributes

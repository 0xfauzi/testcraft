...
from ...ports.telemetry_port import TelemetryPort, MetricValue
...
class CostManager:
    ...
    def __init__(
        ...
        self.per_request_limit = config.get('cost_thresholds', {}).get('per_request_limit', 2.0)
        self.warning_threshold = config.get('cost_thresholds', {}).get('warning_threshold', 1.0)
    ...
    def _load_cost_entries(self) -> List[CostEntry]:
        ...
        try:
            ...
            if self.telemetry:
                with self.telemetry.create_span("cost_manager.load_entries_error") as span:
    ...
    def _save_cost_entries(self) -> None:
        ...
        try:
            ...
            if self.telemetry:
                with self.telemetry.create_span("cost_manager.save_entries_error") as span:
                    span.record_exception(e)
    ...
    def _load_cost_limits(self) -> List[CostLimit]:
        ...
        try:
            with open(self.cost_limits_file, 'r') as f:
            ...
            if self.telemetry:
                with self.telemetry.create_span("cost_manager.load_limits_error") as span:
                    span.record_exception(e)
            return []
    ...
    def _save_cost_limits(self) -> None:
        ...
        try:
            ...
            if self.telemetry:
                with self.telemetry.create_span("cost_manager.save_limits_error") as span:
                    span.record_exception(e)
    ...
    def track_usage(
        ...
        **kwargs: Any
        ...
        with self.telemetry.create_span("cost_manager.track_usage") if self.telemetry else nullcontext() as span:
            if span:
                span.set_attributes({
                    "service": service,
                    "operation": operation,
    ...
    def get_summary(
        ...
        **kwargs: Any
        ...
        if service_filter:
            filtered_entries = [e for e in filtered_entries if e.service in service_filter]
        ...
        return {
            ...
            'usage_stats': {
                ...
                'average_cost_per_operation': total_cost / len(filtered_entries) if filtered_entries else 0
            },
            'summary_metadata': {
                'period': time_period,
    ...
    def set_cost_limit(
        ...
        **kwargs: Any
        ...
        if self.telemetry:
            self.telemetry.increment_counter(
                "cost_tracker.limits_set",
                labels={"period": limit_period}
            )
    ...
    def check_cost_limit(
        ...
        **kwargs: Any
        ...
        if limit_id:
            limits_to_check = [l for l in self.cost_limits if l.id == limit_id]
        else:
            limits_to_check = [l for l in self.cost_limits if l.active]
        ...
        for limit in limits_to_check:
            ...
            if start_time:
                period_entries = [e for e in self.cost_entries if e.timestamp >= start_time]
            ...
            usage_percentage = (period_cost / limit.amount) * 100 if limit.amount > 0 else 0
            ...
            if usage_percentage >= 90:
                warnings.append(f"{limit.period.title()} limit at {usage_percentage:.1f}% (${period_cost:.2f}/${limit.amount:.2f})")
            elif usage_percentage >= 75:
                warnings.append(f"{limit.period.title()} usage at {usage_percentage:.1f}% of limit")
        ...
        try:
            if export_format.lower() == "csv":
                ...
                with open(export_path, 'w', newline='') as csvfile:
                    fieldnames = ['id', 'timestamp', 'service', 'operation', 'cost', 'tokens_used', 'api_calls', 'duration']

...
from ...ports.telemetry_port import TelemetryPort, SpanKind, SpanContext, MetricValue
...
class OtelSpanContextManager:
    ...
    def _sanitize_value(self, key: str, value: Any) -> Optional[AttributeValue]:
...
class OpenTelemetryAdapter:
    ...
    def _create_span_processor(self):
        ...
        otlp_endpoint = (
            self.config.get('backends', {}).get('opentelemetry', {}).get('endpoint') or
            os.getenv('OTEL_EXPORTER_OTLP_ENDPOINT')
        )
        
        if otlp_endpoint:
            # Use OTLP exporter
            headers = self.config.get('backends', {}).get('opentelemetry', {}).get('headers', {})
            insecure = self.config.get('backends', {}).get('opentelemetry', {}).get('insecure', False)
    ...
    def create_span(
    ...
    ) -> ContextManager[OtelSpanContextManager]:
        """
        ...
        if not self.tracer:
            ...
            yield OtelSpanContextManager(None, SpanContext("noop", "noop"), self.anonymize_paths)
            return
        ...
        with self.tracer.start_as_current_span(
            ...
            context_manager = OtelSpanContextManager(span, span_context, self.anonymize_paths)
    ...
    def flush(self, timeout_seconds: Optional[float] = None) -> bool:
        ...
        try:
            ...
            if hasattr(tracer_provider, 'force_flush'):
                tracer_provider.force_flush(timeout_millis=int((timeout_seconds or 5) * 1000))
            
            # Flush metrics
            meter_provider = metrics.get_meter_provider()
            if hasattr(meter_provider, 'force_flush'):
                meter_provider.force_flush(timeout_millis=int((timeout_seconds or 5) * 1000))

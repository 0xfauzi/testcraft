...
class PromptFooConfig:
    ...
    tests: Union[str, List[Dict[str, Any]]]
    ...
    def to_testcraft_format(self) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]], EvaluationConfig]:
        ...
        if isinstance(self.tests, str):
            ...
            for test in self.tests:
                dataset_item = {
                    ...
                    "source_file": test.get("vars", {}).get("source_file", "unknown.py"),
                    "source_content": test.get("vars", {}).get("source_code", ""),
                    "expected_output": test.get("assert", {}).get("contains") if test.get("assert") else None
                }
...
class PromptFooIntegrationAdapter:
    ...
    def __init__(
        ...
        project_root: Path,
        promptfoo_config_path: Optional[Path] = None
    ):
        ...
        for path in [self.prompt_registry_path, self.evaluation_runs_path, self.results_path]:
    ...
    def get_prompt_version(self, prompt_id: str) -> Optional[PromptVersion]:
    ...
    def list_prompt_versions(
        ...
        status_filter: Optional[PromptVersionStatus] = None,
        ...
        if tags_filter:
            versions = [v for v in versions if any(tag in v.tags for tag in tags_filter)]
    ...
    def activate_prompt_version(self, prompt_id: str) -> None:
        ...
        for other_id, other_version in self._prompt_registry.items():
            if (other_version.name == prompt_version.name and 
                other_version.status == PromptVersionStatus.ACTIVE):
                other_version.status = PromptVersionStatus.DEPRECATED
                other_version.updated_at = datetime.now(timezone.utc)
        
        # Activate this version
        prompt_version.status = PromptVersionStatus.ACTIVE
        prompt_version.updated_at = datetime.now(timezone.utc)
    ...
    def complete_evaluation_run(
        ...
        eval_run = self._active_runs[run_id]
        eval_run.status = EvaluationStatus.COMPLETED if not error_message else EvaluationStatus.FAILED
    ...
    def export_promptfoo_results(self, run_id: str, output_path: Path) -> None:
        ...
        promptfoo_results = {
            "version": 2,
            "timestamp": eval_run.completed_at.isoformat() if eval_run.completed_at else None,
            "results": {
                "table": {
                    "head": {
                        "prompts": [{"id": v.id, "label": v.name} for v in eval_run.prompt_variants],
                ...
                "config": {
                    ...
                    "prompts": [{"id": v.id, "content": v.prompt} for v in eval_run.prompt_variants],
                    "tests": [
                        {
                            "vars": test_case,
                            "assert": [{"type": "llm-rubric", "value": "evaluate test quality"}]
                        }
        ...
        for i, test_case in enumerate(eval_run.test_dataset):
            row = {
                "vars": test_case,
                "outputs": []
            }
            ...
            for variant_result in variant_results:
                if variant_result["variant_id"] in [v.id for v in eval_run.prompt_variants]:
                    # Find corresponding evaluation for this test case
                    individual_evals = variant_result.get("individual_evaluations", [])
                    eval_for_case = next(
                        (e for e in individual_evals if e.get("test_case") == test_case.get("name")),
                        None
                    )
                    
                    output = {
                        "pass": eval_for_case.get("acceptance_result", {}).get("passed", False) if eval_for_case else False,
                        "score": eval_for_case.get("llm_judge_result", {}).get("overall_score", 0) if eval_for_case else 0,
                        "namedScores": eval_for_case.get("llm_judge_result", {}).get("scores", {}) if eval_for_case else {},
                        "text": eval_for_case.get("generated_test", "") if eval_for_case else ""
                    }
    ...
    def import_promptfoo_config_and_run(
        ...
        prompt_variants, test_dataset, eval_config = promptfoo_config.to_testcraft_format()
            ...
            for prompt_id, prompt_data in registry_data.items():
                prompt_version = PromptVersion(**{
                    **prompt_data,
                    ...
                    'created_at': datetime.fromisoformat(prompt_data['created_at']) if prompt_data.get('created_at') else None,
                    'updated_at': datetime.fromisoformat(prompt_data['updated_at']) if prompt_data.get('updated_at') else None
                })
    ...
    def _save_prompt_registry(self) -> None:
        ...
        for prompt_id, prompt_version in self._prompt_registry.items():
            ...
            data['created_at'] = prompt_version.created_at.isoformat() if prompt_version.created_at else None
            data['updated_at'] = prompt_version.updated_at.isoformat() if prompt_version.updated_at else None
    ...
    def _save_evaluation_run(self, eval_run: EvaluationRun) -> None:
        ...
        run_data['started_at'] = eval_run.started_at.isoformat() if eval_run.started_at else None
        run_data['completed_at'] = eval_run.completed_at.isoformat() if eval_run.completed_at else None
        
        # Convert PromptVersion objects to dicts
        run_data['prompt_variants'] = [
            {**asdict(pv), 'status': pv.status.value, 
        ...
        for pv_data in run_data.get('prompt_variants', []):
            pv = PromptVersion(**{
                **pv_data,
                ...
                'created_at': datetime.fromisoformat(pv_data['created_at']) if pv_data.get('created_at') else None,
                'updated_at': datetime.fromisoformat(pv_data['updated_at']) if pv_data.get('updated_at') else None
            })

...
class TestMapper:
    ...
    def _identify_coverage_gaps(
        ...
        for source_element in source_elements:
            ...
            if not mapped_tests:
                ...
                gap_info['description'] = f"No test found for {source_element.type} '{source_element.name}'"
            elif len(mapped_tests) == 1:
                gap_info['gap_type'] = 'single_test'
                gap_info['description'] = f"Only one test found for {source_element.type} '{source_element.name}'"
            else:
    ...
    def _generate_test_suggestions(
        self,
        missing_tests: List[TestElement]
    ) -> List[Dict[str, Any]]:
...
'''
    ...
    def _calculate_coverage_percentage(
        ...
        elements_with_tests = sum(
            1 for element in source_elements
            if test_mapping.get(element.name, [])
        )

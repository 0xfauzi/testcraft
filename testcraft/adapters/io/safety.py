...
class SafetyPolicies:
    ...
    @staticmethod
    def validate_file_path(file_path: Path, project_root: Optional[Path] = None) -> None:
        """
        ...
        if file_path.is_absolute():
            if project_root:
                ...
                raise SafetyError(f"Absolute paths not allowed without project root: {file_path}")
        else:

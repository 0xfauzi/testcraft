...
class ArtifactStoreAdapter:
    ...
    def __init__(
        self, 
        base_path: Union[str, Path] = ".testcraft/artifacts",
        ...
        self.cleanup_policy = cleanup_policy or CleanupPolicy(max_age_days=30, max_artifacts=100)
    ...
    def retrieve_artifact(self, artifact_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            raise ArtifactStoreError(f"Failed to retrieve artifact {artifact_id}: {str(e)}")
    ...
    def remove_artifact(self, artifact_id: str) -> bool:
        ...
        try:
            ...
            raise ArtifactStoreError(f"Failed to remove artifact {artifact_id}: {str(e)}")
    ...
    def apply_cleanup_policy(self) -> Dict[str, int]:
        ...
        try:
            ...
            if policy.max_age_days:
                ...
                for artifact_id, metadata in self._metadata.items():
                    if (metadata.timestamp < cutoff_date and 
                        not any(tag in policy.preserve_tags for tag in metadata.tags)):
            ...
            if policy.max_size_mb:
                ...
                total_size = sum(metadata.size_bytes for metadata in self._metadata.values())
    ...
    def get_storage_stats(self) -> Dict[str, Any]:
        ...
        try:
            stats = {
                ...
                "total_size_bytes": sum(metadata.size_bytes for metadata in self._metadata.values()),
                "artifacts_by_type": {},
                "expired_artifacts": 0
            }
            ...
            for metadata in self._metadata.values():
                ...
                stats["artifacts_by_type"][type_name]["size_bytes"] += metadata.size_bytes
        ...
        if self.metadata_file.exists():
            try:
                ...
                for artifact_id, metadata_dict in data.items():
                    self._metadata[artifact_id] = ArtifactMetadata.from_dict(metadata_dict)
            except Exception as e:
...
def store_coverage_report(
    ...
    return store.store_artifact(
        ...
        description=f"Coverage report for test run {test_run_id}" if test_run_id else "Coverage report"
    )
...
def store_generated_test(
    ...
    artifact_id = f"test_{Path(source_file).stem}_{test_run_id}" if test_run_id else None
    return store.store_artifact(

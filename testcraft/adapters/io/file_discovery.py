...
class FileDiscoveryService:
    ...
    def __init__(self, config: Optional[TestPatternConfig] = None):
        """
        ...
        self._exclude_dirs_set: Optional[Set[str]] = None
        self._exclude_patterns_set: Optional[Set[str]] = None
    ...
    def discover_source_files(
        ...
        try:
            ...
            logger.debug(f"Discovering source files in {project_path} with patterns: {patterns}")
            
            # Find files using glob patterns
            for pattern in patterns:
                try:
                    files = list(project_path.glob(f"**/{pattern}"))
                    logger.debug(f"Pattern '{pattern}' found {len(files)} files: {[f.name for f in files]}")
                    
                    for file_path in files:
                        should_include = self._should_include_file(file_path, include_test_files=include_test_files)
                        logger.debug(f"File {file_path.name}: should_include={should_include}")
                        if should_include:
    ...
    def discover_test_files(
        ...
        try:
            ...
            logger.debug(f"Discovering test files in {project_path} with patterns: {patterns}")
    ...
    def discover_all_python_files(
        ...
        try:
            if separate_tests:
                source_files = self.discover_source_files(project_path, include_test_files=False)
                ...
                all_files = self.discover_source_files(project_path, include_test_files=True)
                return {
                    'all_files': all_files,
    ...
    def filter_existing_files(self, file_paths: List[Union[str, Path]]) -> List[str]:
        """
        ...
        for file_path in file_paths:
            ...
            if path.exists() and self._should_include_file(path, include_test_files=True):
                filtered_files.append(str(path.resolve()))
        
        return filtered_files
    
    def _should_include_file(self, file_path: Path, include_test_files: bool = False) -> bool:
        """
    ...
    def _should_include_test_file(self, file_path: Path) -> bool:
        ...
        for pattern in self.exclude_patterns_set:
            ...
            if any(test_keyword in pattern.lower() for test_keyword in ['test_', '_test']):
                continue
            if self._matches_pattern(file_str, pattern):
                return False
    ...
    def _is_test_file(self, file_path: Path) -> bool:
        ...
        if filename.startswith("test_") or filename.endswith("_test.py") or filename in ["conftest.py", "pytest.ini"]:
            return True
    ...
    def get_discovery_stats(self, project_path: Union[str, Path]) -> dict:
        """
        ...
        """
        try:
            discovery_result = self.discover_all_python_files(project_path, separate_tests=True)

...
from ...domain.models import AnalysisReport, CoverageResult, GenerationResult, TestGenerationPlan, RefineOutcome
...
class JsonReportAdapter(ReportPort):
    ...
    def __init__(self, output_base_path: Optional[Union[str, Path]] = None) -> None:
        """
        ...
        **kwargs: Any
        ...
        try:
            report_content = {
                ...
                "files_with_tests": [
                    f for f, has_tests in analysis_data.existing_test_presence.items()
                    if has_tests
                ],
                "files_without_tests": [
                    f for f, has_tests in analysis_data.existing_test_presence.items()
            ...
            if include_recommendations:
                report_content["recommendations"] = self._generate_analysis_recommendations(analysis_data)
            
            summary = f"Analysis of {len(analysis_data.files_to_process)} files: " \
    ...
    def generate_coverage_report(
        ...
        **kwargs: Any
        ...
        """
        try:
            return self._generate_coverage_report_content(coverage_data, style=report_style, **kwargs)
        except Exception as e:
    ...
    def generate_summary_report(
        ...
        **kwargs: Any
        ...
        """
        try:
            return self._generate_summary_report_content(project_data, level=summary_level, **kwargs)
        except Exception as e:
    ...
    def _generate_coverage_report_content(self, coverage_data: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
        ...
        report_content = {
            ...
            "timestamp": datetime.utcnow().isoformat(),
            "style": style,
            "coverage_before": self._serialize_coverage_result(coverage_before) if coverage_before else None,
            "coverage_after": self._serialize_coverage_result(coverage_after) if coverage_after else None,
            "coverage_delta": coverage_delta,
            "files_analyzed": coverage_data.get('files_analyzed', []),
            "missing_coverage_details": coverage_data.get('missing_coverage_details', {})
        }
        
        # Calculate summary metrics
        coverage_summary = self._calculate_coverage_summary(coverage_before, coverage_after)
        
        return {
            "report_content": json.dumps(report_content, indent=2),
            "coverage_summary": coverage_summary,
            "coverage_trends": self._analyze_coverage_trends(coverage_before, coverage_after),
            "coverage_metadata": {
    ...
    def _generate_analysis_report_content(self, data: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
    ...
    def _generate_summary_report_content(self, project_data: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
    ...
    def _generate_generation_report_content(self, data: Dict[str, Any], **kwargs: Any) -> Dict[str, Any]:
        ...
        report_content = {
            ...
            "generation_results": [
                self._serialize_generation_result(result) for result in generation_results
            ],
        ...
        if kwargs.get('verbose', False):
            ...
            report_content["retrieval_diagnostics"] = data.get('retrieval_diagnostics', {})
    ...
    def _serialize_generation_result(self, result: Union[GenerationResult, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _generate_analysis_recommendations(self, analysis: AnalysisReport) -> List[str]:
        ...
        files_without_tests = [
            f for f, has_tests in analysis.existing_test_presence.items()
            if not has_tests
        ]
        ...
        if reason_counts:
            ...
            recommendations.append(f"Priority focus: {reason_counts[top_reason]} files need attention for '{top_reason}'")
        
        return recommendations
    
    def _calculate_coverage_summary(self, before: Optional[CoverageResult], after: Optional[CoverageResult]) -> Dict[str, Any]:
    ...
    def _analyze_coverage_trends(self, before: Optional[CoverageResult], after: Optional[CoverageResult]) -> Dict[str, str]:
        """Analyze coverage trends."""
        if not before or not after:
            return {"trend": "insufficient_data"}
        
        line_trend = "improved" if after.line_coverage > before.line_coverage else \
                    "declined" if after.line_coverage < before.line_coverage else "stable"
        
        branch_trend = "improved" if after.branch_coverage > before.branch_coverage else \
                      "declined" if after.branch_coverage < before.branch_coverage else "stable"
        
        return {
            "line_coverage_trend": line_trend,
            "branch_coverage_trend": branch_trend,
            "overall_trend": "improved" if (line_trend == "improved" or branch_trend == "improved") and \
                            line_trend != "declined" and branch_trend != "declined" else \
                            "declined" if line_trend == "declined" or branch_trend == "declined" else "stable"
        }

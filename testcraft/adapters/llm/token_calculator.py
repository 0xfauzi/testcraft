...
class TokenLimits:
    ...
    max_thinking: Optional[int] = None  # Maximum thinking/reasoning tokens (if supported)
    recommended_thinking: Optional[int] = None  # Conservative thinking tokens
...
class TokenCalculator:
    ...
    def _get_model_limits(self) -> TokenLimits:
        ...
        try:
            ...
            logger.warning(f"Unknown model {self.provider}/{self.model}, using modern defaults")
    ...
    def calculate_max_tokens(
        ...
        if input_length is not None:
            ...
            if available_for_output > 0:
                ...
                logger.warning(f"Input length {input_length} exceeds safe context window")
                final_tokens = min(final_tokens, 512)  # Minimal fallback
        
        # Ensure minimum viable output
        final_tokens = max(final_tokens, 100)
        
        logger.debug(f"Calculated max_tokens: {final_tokens} for {use_case} on {self.provider}/{self.model}")
        return final_tokens
    ...
    def calculate_thinking_tokens(
        ...
        base_thinking = self.limits.recommended_thinking or (self.limits.max_thinking // 2)
        ...
        logger.debug(f"Calculated thinking tokens: {final_thinking} for {use_case} ({complexity_level}) on {self.provider}/{self.model}")
    ...
    def is_reasoning_model(self) -> bool:
        ...
        return (self.provider in ['openai', 'azure-openai'] and 
                any(reasoning_model in self.model for reasoning_model in openai_reasoning_models))

...
from ..widgets import FileTable, Notifications, StatsPanel, Toolbar
...
class GenerateScreen(Screen):
        ...
        with Horizontal(classes="form-row"):
            form_widgets.extend(
                [
                    ...
                    Switch(
                        value=self.config["enable_refinement"], id="enable-refinement"
                    ),
                ]
            )
    ...
    def _update_form_from_config(self) -> None:
        ...
        try:
            ...
            self.query_one("#include-patterns", Input).value = self.config[
                "include_patterns"
            ]
            self.query_one("#exclude-patterns", Input).value = self.config[
                "exclude_patterns"
            ]
            self.query_one("#max-tests", Input).value = str(
                self.config["max_tests_per_file"]
            )
            self.query_one("#enable-refinement", Switch).value = self.config[
                "enable_refinement"
            ]
            ...
            self.config.update(
                {
                    ...
                    "batch_size": int(
                        self.query_one("#batch-size", Input).value or "5"
                    ),
                    "immediate": self.query_one("#immediate-mode", Switch).value,
                    "include_patterns": self.query_one(
                        "#include-patterns", Input
                    ).value,
                    "exclude_patterns": self.query_one(
                        "#exclude-patterns", Input
                    ).value,
                    "max_tests_per_file": int(
                        self.query_one("#max-tests", Input).value or "10"
                    ),
                    "enable_refinement": self.query_one(
                        "#enable-refinement", Switch
                    ).value,
                }
            )
    ...
    async def _start_generation_worker(self) -> None:
        ...
        try:
            ...
            self.app.post_message(ErrorOccurred(f"Generation failed: {e}"))
        finally:
            # Always clean up
            self.operation_active = False
            self._enable_form_controls(True)
    ...
    async def _simulate_generation(self) -> None:
        ...
        for i in range(0, len(files_to_process), batch_size):
            ...
            for file_path in batch:
                ...
                # Update file status
                self.app.post_message(FileStatusChanged(file_path, "running", 0.0))
        ...
        if not (self._current_worker and self._current_worker.is_cancelled):
            self.app.post_message(
                OperationCompleted(
                    "test_generation", True, f"Generated tests for {processed} files"
                )
            )
    ...
    def _validate_config(self) -> bool:
        ...
        try:
            ...
            self.app.post_message(ErrorOccurred(f"Configuration error: {e}"))
            return False
    ...
    def _update_stats(self) -> None:
        ...
        try:
            ...
            stats = {
                ...
                "files_done": len(
                    [f for f in self._file_data.values() if f.get("status") == "done"]
                ),
                "files_failed": len(
                    [f for f in self._file_data.values() if f.get("status") == "failed"]
                ),
                "files_running": len(
                    [
                        f
                        for f in self._file_data.values()
                        if f.get("status") == "running"
                    ]
                ),
                "files_pending": self.total_files - self.files_processed,
                "tests_generated": sum(
                    f.get("tests", 0) for f in self._file_data.values()
                ),
                "total_duration": sum(
                    f.get("duration", 0) for f in self._file_data.values()
                ),
    ...
    def _load_config(self) -> None:
        ...
        self.app.post_message(
            ErrorOccurred(
                "Config loading not implemented yet",
    ...
    def _save_config(self) -> None:
        ...
        self.app.post_message(
            ErrorOccurred(
                "Config saving not implemented yet",
                "This feature will be added in a future update",
            )
        )

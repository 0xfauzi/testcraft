...
class KeyboardManager:
    ...
    def unregister_binding(self, key: str, mode: Optional[NavigationMode] = None) -> None:
        """
    ...
    def get_bindings_for_mode(self, mode: Optional[NavigationMode] = None) -> List[Binding]:
        """
        ...
        # Sort by priority
        bindings.sort(key=lambda b: self.key_bindings[target_mode][b.key].priority, reverse=True)
        return bindings
    
    def get_all_bindings(self) -> Dict[NavigationMode, List[Binding]]:
        """Get all bindings organized by mode."""
        return {
            mode: self.get_bindings_for_mode(mode)
            for mode in NavigationMode
        }

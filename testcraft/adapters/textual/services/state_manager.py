...
class StateManager:
    ...
    def subscribe_many(self, subscriptions: Dict[str, Callable]) -> List[Callable]:
        ...
        """
        return [self.subscribe(key, callback) for key, callback in subscriptions.items()]
    ...
    def reset_state(self, new_state: Optional[Dict[str, Any]] = None) -> None:
        """
        ...
        if self._persist_path:
            try:
                ...
                with open(self._persist_path, 'w') as f:
                    json.dump({
                        "state": self._state,
                        "history": self._history[-10:],  # Save last 10 history entries
                        "timestamp": datetime.now().isoformat()

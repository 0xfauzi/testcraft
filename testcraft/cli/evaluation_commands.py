...
def add_evaluation_commands(app: click.Group) -> None:
    ...
    @click.option(
        "--statistical-testing/--no-statistical-testing",
        default=True,
    ...
    def run_ab_test(
        ...
        try:
            ...
            if not quiet:
                console.print(
                    "✅ [bold green]A/B testing pipeline completed successfully![/bold green]"
                )
    ...
    def run_statistical_analysis(
        ...
        try:
            console.print(
                "📊 [bold blue]Running Statistical Significance Analysis[/bold blue]"
            )
            ...
            with Progress(console=console) as progress:
                task = progress.add_task(
                    "Analyzing statistical significance...", total=100
                )
    ...
    @click.option(
        "--time-window", default=30, type=int, help="Time window in days for analysis"
    )
    @click.option(
        "--evaluation-history",
        type=click.Path(exists=True),
    ...
    def run_bias_detection(
        ...
        try:
            ...
            console.print(
                "✅ [bold green]Bias detection analysis completed![/bold green]"
            )
    ...
    @click.option(
        "--output", "-o", type=click.Path(), help="Output file for campaign results"
    )
    ...
    def run_evaluation_campaign(
        ...
        try:
            console.print(
                "🚀 [bold blue]Starting Comprehensive Evaluation Campaign[/bold blue]"
            )
            ...
            with Progress(console=console) as progress:
                ...
                main_task = progress.add_task(
                    "Running evaluation campaign...", total=len(scenarios)
                )
            ...
            console.print(
                "✅ [bold green]Evaluation campaign completed successfully![/bold green]"
            )
...
def _display_ab_test_summary(config: dict[str, Any]) -> None:
    ...
    for variant in variants:
        prompt_preview = variant.get("prompt", "")[:80] + (
            "..." if len(variant.get("prompt", "")) > 80 else ""
        )
...
• Effect Size: {stat_analysis.get("effect_size", {}).get("interpretation", "unknown")}
        """
        console.print(
            Panel(stat_text, title="Statistical Analysis", border_style="green")
        )
...
• Level: {results.get("confidence_interval", {}).get("confidence_level", 0):.0%}
    ...
    console.print(
        Panel(main_text, title="Statistical Analysis Results", border_style="blue")
    )
...
• Analysis Confidence: {bias_data.get("confidence", 0):.2f}
    ...
    console.print(
        Panel(bias_text, title="Bias Analysis Results", border_style="yellow")
    )
...
• Variance Analysis: {consistency.get("variance_analysis", "N/A")}
    ...
    console.print(
        Panel(consistency_text, title="Consistency Analysis", border_style="green")
    )
    ...
    if immediate_actions or process_improvements:
        ...
        if process_improvements:
            ...
            rec_text += "\n".join(
                f"• {improvement}" for improvement in process_improvements
            )

        console.print(
            Panel(rec_text, title="Mitigation Recommendations", border_style="cyan")
        )
...
def _display_campaign_summary(config: dict[str, Any]) -> None:
    ...
    for i, scenario in enumerate(scenarios):
        table.add_row(
            ...
            + ("..." if len(scenario.get("description", "")) > 50 else ""),
        )
...
• Harness Version: {metadata.get("harness_version", "unknown")}
    ...
    if scenario_results:
        ...
        for scenario in scenario_results:
            ...
            significance = (
                stat_analysis.get("significance_assessment", "N/A")
                if not stat_analysis.get("error")
                else "Error"
            )
            ...
            fairness = (
                f"{bias_analysis.get('fairness_score', 0):.2f}"
                if not bias_analysis.get("error")
                else "Error"
            )
    ...
    if cross_analysis:
        ...
        if insights:
            ...
            console.print(
                Panel(
                    insight_text, title="Cross-Scenario Insights", border_style="green"
                )
            )
    ...
    if recommendations:
        rec_text = "\n".join(f"• {rec}" for rec in recommendations)
        console.print(
            Panel(rec_text, title="Final Recommendations", border_style="cyan")
        )

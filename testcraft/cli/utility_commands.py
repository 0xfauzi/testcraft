...
def add_utility_commands(app: click.Group) -> None:
    ...
    @click.option('--format', '-f', 'output_format', 
                  ...
                  default='toml', help='Configuration format')
    ...
    def init_config(
        ...
        try:
            ...
            ui.display_error(f"Configuration initialization failed: {e}", "Init Config Error")
            if ctx.obj.verbose:
                import traceback
    ...
    def env(
        ...
        try:
            ...
            with ui.create_status_spinner("Collecting environment information..."):
                results = asyncio.run(utility_usecase.get_environment_info(
            ...
            ui.display_error(f"Environment info collection failed: {e}", "Environment Error")
            if ctx.obj.verbose:
                import traceback
                ui.display_info(traceback.format_exc(), "Debug Information")
            sys.exit(1)
    ...
    @click.option('--period', type=click.Choice(['daily', 'weekly', 'monthly']),
                  default='monthly', help='Time period for cost summary')
    ...
    def cost(
        ...
        try:
            ...
            with ui.create_status_spinner("Calculating cost summary..."):
                results = asyncio.run(utility_usecase.get_cost_summary(
                    time_period=period,
    ...
    @click.option('--format', 'output_format',
                  type=click.Choice(['json', 'yaml', 'text']), default='text',
                  help='Output format')
    @click.pass_context
    def debug_state(
        ctx: click.Context,
        ...
        try:
            ...
            with ui.create_status_spinner("Collecting debug information..."):
                results = asyncio.run(utility_usecase.debug_state(
    ...
    @click.option('--persist', is_flag=True, help='Persist state after sync', default=True)
    @click.pass_context
    def sync_state(
        ctx: click.Context,
        reload: bool,
        ...
        try:
            ...
            with ui.create_status_spinner("Synchronizing state..."):
                results = asyncio.run(utility_usecase.sync_state(
                    force_reload=reload,
                    persist_after_sync=persist
                ))
    ...
    @app.command()
    @click.option('--categories', multiple=True, help='Specific state categories to reset')
    @click.option('--backup/--no-backup', default=True, help='Create backup before reset')
    ...
    def reset_state(
        ...
        try:
            ...
            with ui.create_status_spinner("Resetting state..."):
                results = asyncio.run(utility_usecase.reset_state(
...
def display_cost_summary(results: Dict[str, Any]) -> None:
    ...
    if cost_summary:
        ui.display_info(
            f"Total cost: ${cost_summary.get('total_cost', 0):.2f}",
            "Cost Summary"
        )
    ...
    if not limit_status.get('within_limits', True):
        ui.display_warning(
            "Cost limits exceeded - consider adjusting usage",
            "Cost Warning"
        )

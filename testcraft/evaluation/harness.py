...
class TestEvaluationHarness:
    ...
    def __init__(
        self,
        config: Optional[TestCraftConfig] = None,
        ...
        **adapter_kwargs: Any
        ...
        logger.info(f"TestEvaluationHarness initialized for project: {self.project_root}")
    ...
    def run_advanced_ab_testing_pipeline(
        ...
        **kwargs: Any
        ...
        """
        logger.info(f"Running advanced A/B testing pipeline via harness: {len(prompt_variants)} variants")
    ...
    def detect_evaluation_bias(
        self,
        evaluation_history: Optional[List[EvaluationResult]] = None,
        ...
        **kwargs: Any
        ...
        try:
            ...
            for i, scenario in enumerate(scenarios):
                logger.info(f"Running evaluation scenario {i+1}/{len(scenarios)}: {scenario.get('name', f'scenario_{i}')}")
            ...
            if len(campaign_results['scenario_results']) > 1:
                campaign_results['cross_scenario_analysis'] = self._analyze_cross_scenario_results(
                    campaign_results['scenario_results']
                )
            
            # Generate final recommendations
            campaign_results['final_recommendations'] = self._generate_campaign_recommendations(
                campaign_results
            )
    ...
    def _get_recent_evaluation_history(self, days: int) -> List[EvaluationResult]:
        ...
        try:
            ...
            for key, value in all_state.items():
                if isinstance(value, dict) and 'test_id' in value and 'timestamp' in value:
                    try:
                        eval_timestamp = datetime.fromisoformat(value['timestamp'])
                        if eval_timestamp >= cutoff_date:
                            # Convert dict back to EvaluationResult
                            evaluation_results.append(self.evaluator._dict_to_evaluation_result(value))
                    except (ValueError, KeyError) as e:
                        logger.debug(f"Skipping invalid evaluation record: {e}")
                        continue
            
            logger.info(f"Retrieved {len(evaluation_results)} evaluations from the last {days} days")
    ...
    def _analyze_cross_scenario_results(self, scenario_results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            for scenario in scenario_results:
                for variant_eval in scenario.get('variant_evaluations', []):
                    ...
                    variant_performances[variant_id].append(stats.get('mean_llm_score', 0))
            ...
            for variant_id, scores in variant_performances.items():
                if len(scores) > 1:
                    ...
                    cross_analysis['consistency_analysis'][variant_id] = {
                        ...
                        'consistency_rating': 'high' if variance < 0.5 else 'medium' if variance < 1.0 else 'low'
                    }
            
            # Overall cross-scenario insights
            cross_analysis['insights'] = self._generate_cross_scenario_insights(cross_analysis)
    ...
    def _generate_cross_scenario_insights(self, cross_analysis: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if most_consistent[0]:
                insights.append(f"Most consistent variant across scenarios: {most_consistent[0]}")
            ...
            if best_performer[0]:
                insights.append(f"Highest average performance: {best_performer[0]} (score: {best_performer[1].get('mean_performance', 0):.2f})")
            
            # General insights
            high_consistency_count = sum(
                1 for data in consistency_data.values()
                if data.get('consistency_rating') == 'high'
            )
            
            if high_consistency_count > len(consistency_data) * 0.5:
                insights.append("Good overall consistency detected across variants and scenarios")
            else:
                insights.append("Variable performance detected - consider scenario-specific optimization")
    ...
    def _generate_campaign_recommendations(self, campaign_results: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if successful_scenarios == len(scenario_results):
                recommendations.append("All evaluation scenarios completed successfully")
            else:
                recommendations.append(f"{successful_scenarios}/{len(scenario_results)} scenarios completed successfully - review failed scenarios")
            ...
            significant_results = sum(
                1 for scenario in scenario_results
                if scenario.get('statistical_analysis', {}).get('significance_assessment') in ['significant', 'highly_significant']
            )
            
            if significant_results > 0:
                recommendations.append(f"{significant_results} scenarios show statistically significant results - findings are reliable")
            else:
                recommendations.append("No statistically significant differences detected - consider larger sample sizes or different evaluation criteria")
            ...
            if bias_issues > 0:
                recommendations.append(f"Potential bias detected in {bias_issues} scenarios - review evaluation methodology")
            
            # General recommendations
            recommendations.append("Review detailed scenario results for specific optimization opportunities")
            
        except Exception as e:
            logger.warning(f"Failed to generate campaign recommendations: {e}")
            recommendations.append("Campaign completed - review results manually due to recommendation generation error")

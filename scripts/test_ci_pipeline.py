...
class LocalCITester:
    ...
    def _run_command(self, cmd: List[str], cwd: Optional[Path] = None) -> Tuple[bool, Dict]:
        ...
        try:
            ...
            return False, {"error": "Command timed out after 5 minutes", "command": " ".join(cmd)}
    ...
    def _run_mypy_check(self) -> Tuple[bool, Dict]:
        ...
        return self._run_command(["uv", "run", "mypy", "testcraft/", "--show-error-codes", "--pretty"])
    ...
    def _run_security_scan(self) -> Tuple[bool, Dict]:
        ...
        # Run safety check
        return self._run_command(["uv", "run", "safety", "check", "--json", "--output", "local-safety-report.json"])
    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if "Lint Check (ruff)" in failed_checks:
            recommendations.append("🔧 Fix linting issues by running: uv run ruff check . --fix")
        
        if "Format Check (ruff)" in failed_checks or "Format Check (black)" in failed_checks:
            recommendations.append("🎨 Fix formatting by running: uv run black . && uv run ruff format .")
        ...
        if "Test Suite (pytest)" in failed_checks:
            recommendations.append("🧪 Fix failing tests - check the pytest output for details")
        
        if "Documentation Check" in failed_checks:
            recommendations.append("📚 Fix documentation issues - check README.md and docstrings")
        
        if "Prompt Regression Test" in failed_checks:
            recommendations.append("📝 Fix prompt-related issues - check prompt registry and evaluation system")
        
        if "Security Scan" in failed_checks:
            recommendations.append("🔒 Review security scan results and update vulnerable dependencies")
        
        recommendations.append("🔄 Re-run this script after fixing issues to verify they're resolved")
...
def main():
    ...
    parser.add_argument(
        "--check",
        choices=["lint", "format", "black", "type", "test", "docs", "prompt", "security"],
    ...
    if args.list_checks:
        ...
        checks = ["lint", "format", "black", "type", "test", "docs", "prompt", "security"]
        for check in checks:
            print(f"  • {check}")
        return 0

...
class DocumentationChecker:
    ...
    def _check_readme_syntax(self) -> bool:
        ...
        if missing_sections:
            self.warnings.append(
                f"README.md missing recommended sections: {missing_sections}"
            )
    ...
    def _check_code_examples(self) -> bool:
        ...
        for i, code_block in enumerate(python_blocks):
            ...
            if any(
                cmd in code_block for cmd in ["git clone", "cd ", "uv ", "testcraft "]
            ):
                continue
            ...
            try:
                if (
                    "def " in code_block
                    or "import " in code_block
                    or "from " in code_block
                ):
        ...
        if syntax_errors:
            self.warnings.append(
                f"Python syntax issues in README code examples: {syntax_errors}"
            )
    ...
    def _check_project_structure(self) -> bool:
        ...
        print("  ✓ All documented directories and key files exist")
        return True
        ...
        if cli_main_path.exists():
            try:
                ...
                if "@click." not in cli_content and "@app." not in cli_content:
                    self.warnings.append(
                        "CLI main.py doesn't appear to use Click framework"
                    )
    ...
    def _check_api_coverage(self) -> bool:
        ...
        if undocumented_modules:
            ...
            self.warnings.append(
                f"Modules without docstrings: {undocumented_modules[:5]}{'...' if len(undocumented_modules) > 5 else ''}"
            )

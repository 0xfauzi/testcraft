...
class CIArtifactManager:
    ...
    def collect_security_artifacts(self) -> dict[str, Any]:
        ...
        print(
            f"  ✓ Collected {len(artifacts['security_files'])} security artifact files"
        )
        return artifacts
    ...
    def compress_artifacts(self, max_size_mb: int = 50) -> dict[str, Any]:
            ...
            if artifact_path.is_file() and not artifact_path.name.endswith(
                ...
                if file_size > max_size_bytes:
                    ...
                    try:
                        ...
                        compression_info["compressed_files"].append(
                            str(compressed_path)
                        )
                        compression_info["size_savings"][str(artifact_path)] = {
                            ...
                            "savings_percent": round(
                                (file_size - compressed_size) / file_size * 100, 1
                            ),
                        }

                        artifact_path.unlink()  # Remove original
                        print(
                            f"  ✓ Compressed {artifact_path.name}: {file_size // 1024}KB → {compressed_size // 1024}KB"
                        )
        ...
        total_savings = sum(
            info["savings_bytes"] for info in compression_info["size_savings"].values()
        )
        if total_savings > 0:
    ...
    def cleanup_old_artifacts(self, retention_days: int = 7) -> dict[str, Any]:
        ...
        if cleanup_info["space_freed"] > 0:
            print(
                f"  ✓ Cleaned {len(cleanup_info['cleaned_files'])} files, freed {cleanup_info['space_freed'] // 1024}KB"
            )
        else:
    ...
    def generate_artifact_report(self) -> dict[str, Any]:
        ...
        print(
            f"  📏 Total artifact size: {report['storage_info']['total_size_mb']}MB ({file_count} files)"
        )
    ...
    def _categorize_artifacts(self) -> dict[str, dict[str, Any]]:
        ...
        categories = {
            "test_results": {
                "pattern": ["test-results", "pytest"],
                "count": 0,
                "size": 0,
            },
            "coverage_reports": {
                "pattern": ["coverage", "htmlcov"],
                "count": 0,
                "size": 0,
            },
            "prompt_regression": {
                "pattern": ["prompt-regression", "prompt"],
                "count": 0,
                "size": 0,
            },
            "security_scans": {
                "pattern": ["security", "safety", "bandit"],
                "count": 0,
                "size": 0,
            },
        ...
        for artifact_path in self.artifacts_base.rglob("*"):
            if artifact_path.is_file():
                ...
                for category, info in categories.items():
                    ...
                    if any(
                        pattern in str(artifact_path).lower()
    ...
    def run_full_collection(
        self, compress: bool = True, cleanup: bool = True
    ) -> dict[str, Any]:
        """Run full artifact collection and management."""
        print("🚀 Starting full CI artifact collection...")

        results = {"start_time": datetime.now().isoformat(), "operations": []}
        ...
        if compress:
            results["operations"].append(
                {"operation": "compress_artifacts", "result": self.compress_artifacts()}
            )
...
def main():
    ...
    parser.add_argument(
        "--no-compress", action="store_true", help="Skip compression of large files"
    )
    parser.add_argument(
        "--no-cleanup", action="store_true", help="Skip cleanup of old artifacts"
    )
    parser.add_argument(
        "--retention-days",
        type=int,
    ...
    if args.collect_only:
        ...
        manager.run_full_collection(
            compress=not args.no_compress, cleanup=not args.no_cleanup
        )

...
from testcraft.ports.telemetry_port import SpanKind, MetricValue
...
class TestNoOpTelemetryAdapter:
    ...
    def test_metric_operations(self):
        ...
        adapter.record_metrics([
            MetricValue("metric1", 1.0),
            MetricValue("metric2", 2.0)
        ])
...
class TestOpenTelemetryAdapter:
    ...
    def test_initialization_without_otel(self):
        ...
        with patch('testcraft.adapters.telemetry.opentelemetry_adapter.OPENTELEMETRY_AVAILABLE', False):
            adapter = OpenTelemetryAdapter(config)
            assert not adapter.is_enabled()
            assert adapter.tracer is None
            assert adapter.meter is None

    @patch('testcraft.adapters.telemetry.opentelemetry_adapter.OPENTELEMETRY_AVAILABLE', True)
    def test_initialization_with_otel(self):
        ...
        with patch('testcraft.adapters.telemetry.opentelemetry_adapter.trace') as mock_trace, \
             patch('testcraft.adapters.telemetry.opentelemetry_adapter.metrics') as mock_metrics:
    ...
    def test_span_context_manager_sanitization(self):
        ...
        from testcraft.adapters.telemetry.opentelemetry_adapter import OtelSpanContextManager
        ...
        manager.set_attribute("code_content", "def sensitive_function():\n" + "    pass\n" * 100)
        mock_span.set_attribute.assert_called()

...
from testcraft.ports.telemetry_port import SpanKind, TelemetryPort
...
class TestCoverageUseCase:
    ...
    def mock_coverage_port(self):
        ...
        mock.measure_coverage.return_value = {
            "/test/file1.py": CoverageResult(
                line_coverage=0.85, branch_coverage=0.75, missing_lines=[10, 15, 20]
            ),
            "/test/file2.py": CoverageResult(
                line_coverage=0.92, branch_coverage=0.88, missing_lines=[5]
            ),
        }
        ...
        mock.identify_gaps.return_value = {"/test/file1.py": [10, 15, 20]}
        return mock
    ...
    def mock_file_discovery_service(self):
        ...
        mock.discover_source_files.return_value = [
            "/test/module1.py",
            "/test/module2.py",
        ]
        mock.discover_test_files.return_value = ["/test/test_module1.py"]
        mock.filter_existing_files.return_value = [
            "/test/module1.py",
            "/test/module2.py",
        ]
        return mock
    ...
    async def test_measure_and_report_with_specific_files(
        ...
        # Act
        result = await coverage_usecase.measure_and_report(
            project_path=sample_project_path, source_files=source_files
        )
    ...
    async def test_coverage_measurement_failure(
        self, coverage_usecase, sample_project_path, mock_coverage_port
    ):
        """Test handling of coverage measurement failures."""
        # Arrange
        mock_coverage_port.measure_coverage.side_effect = Exception(
            "Coverage tool failed"
        )

        # Act & Assert
        with pytest.raises(CoverageUseCaseError) as exc_info:
            await coverage_usecase.measure_and_report(project_path=sample_project_path)
    ...
    def test_file_discovery_service_integration(
        self, coverage_usecase, mock_file_discovery_service
    ):
    ...
    async def test_batched_coverage_measurement(
        ...
        mock_coverage_port.measure_coverage.side_effect = [
            {
                "/file1.py": CoverageResult(
                    line_coverage=0.8, branch_coverage=0.7, missing_lines=[]
                )
            },
            {
                "/file2.py": CoverageResult(
                    line_coverage=0.9, branch_coverage=0.8, missing_lines=[]
                )
            },
            {
                "/file3.py": CoverageResult(
                    line_coverage=0.85, branch_coverage=0.75, missing_lines=[]
                )
            },
        ]
    ...
    @pytest.mark.asyncio
    async def test_get_coverage_history(self, coverage_usecase, mock_state_port):
    ...
    @pytest.mark.asyncio
    async def test_get_coverage_history_empty(self, coverage_usecase, mock_state_port):
    ...
    def test_calculate_coverage_delta_with_previous_data(
        self, coverage_usecase, mock_state_port
    ):
    ...
    def test_calculate_coverage_delta_declining(self, coverage_usecase):
    ...
    def test_calculate_coverage_delta_stable(self, coverage_usecase):
    ...
    async def test_telemetry_recording(
        self, coverage_usecase, sample_project_path, mock_telemetry_port
    ):
        ...
        mock_telemetry_port.create_span.assert_called_once_with(
            "coverage_measure_and_report", kind=SpanKind.INTERNAL, attributes=ANY
        )
    ...
    async def test_state_recording(
        self, coverage_usecase, sample_project_path, mock_state_port
    ):
    ...
    async def test_error_handling_in_state_recording(
        ...
        )  # Should still succeed despite state recording failure
    ...
    async def test_nonexistent_project_path(
        self, mock_coverage_port, mock_state_port, mock_telemetry_port
    ):
        ...
        with pytest.raises(CoverageUseCaseError):
            await coverage_usecase.measure_and_report(project_path=nonexistent_path)
    ...
    async def test_coverage_measurement_with_no_files_found(
        self, coverage_usecase, tmp_path, mock_coverage_port
    ):
        ...
        # Act
        result = await coverage_usecase.measure_and_report(project_path=empty_project)

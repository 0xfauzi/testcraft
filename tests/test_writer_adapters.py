...
from testcraft.adapters.io.writer_ast_merge import WriterASTMergeAdapter, WriterASTMergeError
from testcraft.adapters.io.safety import SafetyPolicies, SafetyError
...
class TestWriterAppendAdapter:
    ...
    def setup_method(self):
        ...
        self.dry_run_adapter = WriterAppendAdapter(project_root=self.temp_dir, dry_run=True)
...
"""
        ...
        with patch.object(self.adapter, '_format_content', return_value=content.strip()):
    ...
    def test_append_to_existing_file(self):
        ...
        with patch.object(self.adapter, '_format_content', return_value=existing_content + new_content):
...
"""
        ...
        with patch.object(self.adapter, '_format_content', return_value=test_content.strip()):
            result = self.adapter.write_test_file(test_file, test_content, source_file="mymodule.py")
...
class TestWriterASTMergeAdapter:
    ...
    def setup_method(self):
        ...
        self.dry_run_adapter = WriterASTMergeAdapter(project_root=self.temp_dir, dry_run=True)
...
"""
        ...
        with patch.object(self.adapter, '_format_content', return_value="merged content"):
            result = self.adapter.write_file(test_file, new_content)
    ...
    def test_dry_run_with_diff(self):
        ...
        with patch.object(self.adapter, '_merge_content', return_value=new_content):
            with patch.object(self.adapter, '_format_content', return_value=new_content):
...
"""
        ...
        with patch.object(self.adapter, '_format_content', return_value=content.strip()):
            result = self.adapter.write_test_file(test_file, content, source_file="mymodule.py")
        ...
        assert 'helper_function' not in test_functions  # Should not include non-test functions

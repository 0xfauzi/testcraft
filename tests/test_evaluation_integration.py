...
from testcraft.adapters.evaluation.main_adapter import (
    TestcraftEvaluationAdapter,
    EvaluationError
)
...
def process_data_batch(
    ...
    async def test_end_to_end_ab_testing_pipeline(self, mock_llm_responses):
        ...
        with patch.object(
            ...
            assert stat_analysis["significance_assessment"] == "statistically_significant"
    ...
    def test_artifact_storage_integration(self):
        ...
        test_result = EvaluationResult(
            ...
            llm_judge_result=LLMJudgeResult(
                scores={"correctness": 4.5, "coverage": 4.0, "clarity": 4.8, "safety": 5.0},
                rationales={
        ...
        with patch.object(
            ...
            for i in range(20):
                eval_result = EvaluationResult(
                    ...
                    generated_test=f"def test_function_{i}(): assert True" + " " * (i * 10),  # Varying lengths
                    acceptance_result=AcceptanceResult(
                        passed=True,
                        error_message=None,
                        execution_time=0.1,
                    ...
                    llm_judge_result=LLMJudgeResult(
                        scores={
                            "correctness": 4.0 + (len(f"def test_function_{i}(): assert True" + " " * (i * 10)) / 1000),  # Length bias
                            "coverage": 4.0,
                            "clarity": 4.5,
                            "safety": 5.0
                        },
            ...
            assert consistency["drift_detected"] == False
            ...
            assert any("length" in action.lower() for action in mitigation["immediate_actions"])
...
class TestEvaluationErrorHandling:
    ...
    def test_invalid_prompt_variant_handling(self):
        ...
        with pytest.raises(ValueError, match="Invalid prompt variant"):
            self.harness.run_advanced_ab_testing_pipeline(
                prompt_variants=invalid_variants,
    ...
    def test_empty_test_dataset_handling(self):
        ...
        prompt_variants = [{
            "id": "test_variant",
            "prompt": "Generate tests for: {{ source_code }}"
        }]

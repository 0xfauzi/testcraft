...
class TestAnalyzeUseCase:
    ...
    @pytest.mark.asyncio
    async def test_analyze_generation_needs_basic(
        self, analyze_usecase, sample_project_path, mock_ports
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_with_target_files(
        self, analyze_usecase, sample_project_path, mock_ports
    ):
        ...
        result = await analyze_usecase.analyze_generation_needs(
            sample_project_path, target_files=target_files
        )
    ...
    @pytest.mark.asyncio
    async def test_analyze_existing_test_detection(
        self, analyze_usecase, sample_project_path, mock_ports
    ):
    ...
    @pytest.mark.asyncio
    async def test_analyze_processing_reasons(
        self, analyze_usecase, sample_project_path, mock_ports
    ):
        ...
        for _file_path, reason in result.reasons.items():
            assert isinstance(reason, str)
            assert len(reason) > 0
            # Reason should mention coverage or tests
            assert any(
                keyword in reason.lower()
                for keyword in ["coverage", "test", "threshold"]
            )
    ...
    def test_analyze_usecase_initialization(self, mock_ports):
        ...
        config = {"coverage_threshold": 0.75, "custom_setting": "test_value"}
    ...
    @pytest.mark.asyncio
    async def test_build_processing_reasons_error_handling(
        self, analyze_usecase, sample_project_path, mock_ports
    ):
        ...
        with patch.object(
            analyze_usecase,
            "_get_processing_reason",
            side_effect=Exception("Reason error"),
        ):
            with pytest.raises(AnalyzeUseCaseError) as exc_info:
    ...
    @pytest.mark.asyncio
    async def test_build_test_presence_info_error_handling(
        self, analyze_usecase, sample_project_path, mock_ports
    ):
        ...
        with patch.object(
            analyze_usecase,
            "_has_existing_tests",
            side_effect=Exception("Test presence error"),
        ):
            with pytest.raises(AnalyzeUseCaseError) as exc_info:
                await analyze_usecase.analyze_generation_needs(sample_project_path)

...
from testcraft.adapters.io.subprocess_safe import SubprocessExecutionError, SubprocessTimeoutError
...
class TestRunFormatterSafe:
    ...
    def test_run_formatter_safe_custom_timeout(self, mock_safe):
        ...
        with patch('tempfile.NamedTemporaryFile') as mock_temp, \
            ...
            mock_safe.assert_called_once_with(['python', '-m', 'black', '/tmp/test.py'], timeout=60)
            assert result == "formatted content"
...
class TestPythonFormatters:
    ...
    def test_format_python_content_default_timeout(self):
        ...
        with patch('testcraft.adapters.io.python_formatters.run_isort_safe') as mock_isort, \
             patch('testcraft.adapters.io.python_formatters.run_black_safe') as mock_black:
...
class TestIntegration:
    ...
    def test_formatter_command_construction(self):
        ...
        with patch('testcraft.adapters.io.python_formatters.run_subprocess_safe') as mock_safe:
            mock_safe.return_value.__enter__.return_value = ("", "")
            ...
            with patch('tempfile.NamedTemporaryFile') as mock_temp, \
                ...
                run_black_safe("code")
                mock_safe.assert_called_with(['python', '-m', 'black', '--quiet', '/tmp/test.py'], timeout=30)
                
                mock_safe.reset_mock()
                
                # Test isort command construction
                run_isort_safe("code")
                mock_safe.assert_called_with(['python', '-m', 'isort', '--quiet', '/tmp/test.py'], timeout=30)

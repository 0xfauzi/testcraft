...
class TestTestCraftConfig:
    ...
    def test_coverage_validation_failure(self):
        ...
        with pytest.raises(ValidationError) as exc_info:
            TestCraftConfig(coverage={
                'minimum_line_coverage': 70.0,
                'regenerate_if_below': 80.0
            })
        assert 'regenerate_if_below cannot be higher than minimum_line_coverage' in str(exc_info.value)
        ...
        assert 'minimum_branch_coverage cannot be higher than minimum_line_coverage' in str(exc_info.value)
    
    def test_cost_validation_failure(self):
        """Test cost validation failures."""
        with pytest.raises(ValidationError) as exc_info:
            TestCraftConfig(cost_management={
                'cost_thresholds': {
                    'per_request_limit': 1.0,
                    'warning_threshold': 2.0
                }
            })
        assert 'warning_threshold cannot be higher than per_request_limit' in str(exc_info.value)
    
    def test_quality_score_validation(self):
        """Test quality score validation."""
        # Valid scores
        config = TestCraftConfig(quality={
...
class TestConfigLoader:
    ...
    def test_parse_env_value(self):
        ...
        assert loader._parse_env_value('item1, item2, item3') == ['item1', 'item2', 'item3']
    ...
    def test_priority_order(self):
        ...
        env_vars = {
            'TESTCRAFT_COVERAGE__MINIMUM_LINE_COVERAGE': '80'
        }
    ...
    def test_validation_error_handling(self):
        ...
        invalid_config = {
            'coverage': {
                'minimum_line_coverage': 'invalid'  # Should be float
            }
        }

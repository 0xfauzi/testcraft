...
from testcraft.adapters.io.file_discovery import FileDiscoveryService, FileDiscoveryError
...
class TestFileDiscoveryService:
    ...
    def test_discover_all_python_files_separate(self, minimal_config, sample_project):
        ...
        assert result['total_files'] == len(result['source_files']) + len(result['test_files'])

    def test_discover_all_python_files_combined(self, file_discovery_service, sample_project):
    ...
    def test_custom_test_patterns(self, file_discovery_service, sample_project):
        ...
        test_files = file_discovery_service.discover_test_files(
            sample_project,
            test_patterns=['test_*.py', '*_test.py', 'spec_*.py']
        )
    ...
    def test_is_test_file(self, file_discovery_service):
        ...
        assert file_discovery_service._is_test_file(Path("/project/tests/integration.py"))
    ...
    def test_matches_pattern(self, file_discovery_service):
        ...
        assert file_discovery_service._matches_pattern("migrations/001.py", "migrations/*")
        assert file_discovery_service._matches_pattern("path/to/deprecated/file.py", "deprecated")
        
        assert not file_discovery_service._matches_pattern("module.py", "test_*")
        assert not file_discovery_service._matches_pattern("test_module.py", "*_test.py")
    ...
    def test_get_discovery_stats(self, minimal_config, sample_project):
        ...
        assert stats['total_files'] == stats['source_files_count'] + stats['test_files_count']

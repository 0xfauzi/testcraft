...
""")
        ...
        mock_telemetry.create_child_span = MagicMock(
...
""")
        ...
        config = RefineConfig(
            strict_assertion_preservation=True, report_suspected_prod_bugs=True
        )
...
""",
            ...
            assert "calculate function returns 6" in result.get(
                "suspected_prod_bug", ""
            )
            assert result.get("bug_report_created") is True
    ...
    def test_refine_adapter_with_strict_preservation(self):
        ...
        mock_llm = MagicMock()
        adapter = RefineAdapter(llm=mock_llm, config=config)
    ...
    def test_refine_adapter_handles_suspected_bug_response(self, tmp_path):
        ...
        mock_llm = MagicMock()
        mock_llm.refine_content = MagicMock(
            return_value={

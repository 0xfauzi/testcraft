...
'''
    ...
    def test_list_artifacts_by_type(self) -> None:
        ...
        coverage_id = self.store.store_artifact(ArtifactType.COVERAGE_REPORT, {"test": 1})
        test_id = self.store.store_artifact(ArtifactType.GENERATED_TEST, "test content")
        
        coverage_artifacts = self.store.list_artifacts(artifact_type=ArtifactType.COVERAGE_REPORT)
        test_artifacts = self.store.list_artifacts(artifact_type=ArtifactType.GENERATED_TEST)
    ...
    def test_list_artifacts_by_tags(self) -> None:
        ...
        id1 = self.store.store_artifact(ArtifactType.COVERAGE_REPORT, {"test": 1}, tags=["tag1", "tag2"])
        id2 = self.store.store_artifact(ArtifactType.GENERATED_TEST, "content", tags=["tag1", "tag3"])
        id3 = self.store.store_artifact(ArtifactType.LLM_RESPONSE, {"llm": "data"}, tags=["tag2"])
    ...
    def test_cleanup_expired(self) -> None:
        ...
        expired_id1 = self.store.store_artifact(ArtifactType.COVERAGE_REPORT, {"test": 1})
        expired_id2 = self.store.store_artifact(ArtifactType.GENERATED_TEST, "test")
        valid_id = self.store.store_artifact(ArtifactType.LLM_RESPONSE, {"valid": "data"})
        
        # Manually set some to expired
        self.store._metadata[expired_id1].expires_at = datetime.utcnow() - timedelta(hours=1)
        self.store._metadata[expired_id2].expires_at = datetime.utcnow() - timedelta(hours=2)
    ...
    def test_get_storage_stats(self) -> None:
        ...
        self.store.store_artifact(ArtifactType.COVERAGE_REPORT, {"test": 1}, tags=["tag1"])
        self.store.store_artifact(ArtifactType.GENERATED_TEST, "test content", tags=["tag2"])
        expired_id = self.store.store_artifact(ArtifactType.LLM_RESPONSE, {"llm": "data"})
        
        # Make one expired
        self.store._metadata[expired_id].expires_at = datetime.utcnow() - timedelta(hours=1)

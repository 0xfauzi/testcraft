...
class TestRefineAdapter:
    ...
    def test_refine_from_failures_success_first_iteration(self, mock_run_command, mock_path):
    ...
    def test_refine_from_failures_llm_error(self, mock_path):
        ...
        with patch.object(self.mock_llm, 'generate', side_effect=Exception("LLM error")):
            result = self.adapter.refine_from_failures(
                test_file="test_example.py",
                failure_output="AssertionError"
            )
    ...
    def test_refine_from_failures_no_change_detection(self, mock_path):
        ...
        assert result["success"] is False
        assert "No changes made" in result["error"] or "no changes" in result["error"].lower()
...
The issue was with the assertion."""
    ...
    def test_apply_refinement_safely_success(self, mock_path_class):
        ...
        mock_backup_file.write_text.assert_called_once_with("original content", encoding="utf-8")
        # Verify refined content was written
        mock_test_file.write_text.assert_called_once_with(refined_content, encoding="utf-8")
...
class TestRefineAdapterIntegration:
    ...
    def test_full_refinement_flow_mock(self):
        ...
        with patch('testcraft.adapters.refine.main_adapter.Path') as mock_path, \
             patch('testcraft.adapters.refine.main_adapter.run_subprocess_simple') as mock_run_command:

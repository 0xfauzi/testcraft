...
async def demo_minimal_file_tracking():
        ...
        for file_path in sample_files:
            ...
            if success:
                ...
                file_tracker.update_generation_result(
                    file_path, True, random.randint(3, 8)
                )
            else:
                file_tracker.update_file_status(
                    file_path,
    ...
    ui.console.print(
        f"generated [accent]{stats['total_tests_generated']}[/] tests in [muted]{stats['total_duration']:.1f}s[/]\n"
    )
...
def demo_minimal_success():
    ...
    summary_data = {
        ...
        "metrics": {
            "generation": {"duration": 12.3, "items_processed": 4, "success_rate": 0.75}
        },
    }
...
def demo_minimal_errors():
    ...
    ui.display_error_with_suggestions(
        ...
        ["check toml syntax", "verify required fields", "run testcraft init-config"],
        "config error",
    )
